...
namespace sup {
struct Saturating {} constexpr saturating;
...
public:
	...
	: raw{SUP_FWD(us)...} {}
	template<RangeOf<T> R> constexpr explicit Arr(R&& range, Saturating)
	: Arr{std::make_index_sequence<n>{}, std::ranges::begin(range)} {
		assert(std::ranges::size(range) >= n);
	}
	template<RangeOf<T> R> requires (Constructible<T> || !n)
	constexpr explicit Arr(R&& range) {
		if constexpr (n != 0)
			for (auto src = std::ranges::begin(range); auto& t: raw) t = *src++;
	}
